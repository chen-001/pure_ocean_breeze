...
> 1. download_single_daily和database_update_daily_files新增更新pe_ttm部分
> 2. read_daily新增读取pettm数据
> 3. 修复了pure_moon回测时组数非10组时的画图错误
...
> 2. 修复了pure_dawn中，使用whole_cross时的bug
> 3. 给clip_mad增加了keep_trend参数，用于将超出3倍mad范围外的异常值替换为均匀分布在3倍～3.5倍区间内的值
> 4. 给pure_moon新增了group_rets_skews和group_rets_skew，用于展示不同分组之间收益的偏度
> 5. 修复了pure_moonnight中comment_yearly方法的结果错误
> 6. 给pure_fall_frequent的from_cross_via_zip装饰器新增了对返回dataframe的支持
...
> 2. 删去了一些不必要的加载
> 3. 给pure_moonnight、test_on_index_four_out、test_on_300500、test_on_index_four、pure_fama、follow_tests新增了total_cap参数，表示加权和行业市值中性化时使用总市值，而非流通市值
> 4. 将test_on_index_four_out、test_on_300500、test_on_index_four、pure_fama、follow_tests中的value_weighted或index_member_value_weighted参数默认值修改为1
> 5. 新增了symmetrically_orthogonalize函数，用于给一组因子做对称正交
> 6. 新增了icir_weight函数，用于给一组因子做ICIR加权合成
> 7. 新增了scipy_weight函数，用于给一组因子做优化求解版本的ICIR加权合成
> 8. 新增pure_moon_b、pure_week_b、pure_moon_c、pure_week_c类，用于辅助回测


* v3.9.3 — 2023.4.24

> 1. 给pure_moonnight回测绩效，新增“多头超均收益”和“多头收益占比”两个指标，删去了原有的“中心化后偏度”和“二阶自相关性”指标
> 2. 给test_on_index_four_out、test_on_300500、test_on_index_four新增了value_weighted参数，用来决定指数成分股内的测试是否用流通市值加权
> 3. 给follow_tests新增index_member_value_weighted参数，用来决定指数成分股内的测试是否用流通市值加权
...
> 1. 修复了在python3.8以下版本报错的bug

* v3.8.3 — 2023.3.16

> 1. 改变调用加载方式，不初始化也可以使用部分功能
> 1. 引入Dict类型代替dict
* v3.8.2 — 2023.3.15
...
> 1. 给pure_dawn新增了for_cross_via_zip的参数，用于处理返回多个series的情形
* v3.7.8 — 2023.2.27
...
> 1. 对daily_factor_on300500删去了zz800参数，新增支持多个参数同时为1，此时为合并股票池
> 1. 修复了remove_unavailable对月频因子报错的bug
> 1. 给test_on_300500和test_on_index_four新增了group_num参数，用于指定分组回测时的分组数量
> 1. 修复了do_on_dfs装饰器中，第一个参数不能为None的bug
* v3.7.1 — 2023.1.22
