...
def abm_step_two(code,date,func,MOMENTUM_AGENT_SPEC,RUST_FEATURE_CONFIG,VECTOR_TO_SCALAR_CONFIG,GLOBAL_FEATURE_CONFIG,NS_PER_SEC):
    ...
    def _compute_target_two_part(
        ...
        event_arr, event_cols, grid_arr, grid_cols, param_arr, param_cols = rp.compute_agent_trading_features_multi(
        ...
        direct_scalar_df = rp.compute_agent_trading_daily_feature_table_multi(
    ...
    def run_momentum_all_agents_two_part_analysis(
        code: str = "000001",
        date: int = 20220819,
    ) -> Dict[str, Any]:
    ...
    def df_tolist(df: pd.DataFrame, drop_cols: List[str]) -> tuple[list]:
    ...
    res = run_momentum_all_agents_two_part_analysis(code, date)
    res_sim_summary, names_sim_summary = df_tolist(res["sim_summary_df"], ["agent_idx", "agent_name", "lookback_ms"])
    res_per_agent, names_per_agent = df_tolist(res["per_agent_vector_to_scalar_df"], ["agent_idx", "agent_name"])
    res_global_interaction, names_global_interaction = (
        res["global_interaction_df"].iloc[0, :].tolist(),
        res["global_interaction_df"].columns.tolist(),
    )
    ...
    return res0, names, res

def abm_generate_names(agent_names):
    """Generate feature names based on agent names."""
    
    # Basic stats suffixes for each agent
    basic_suffixes = [
        '_n_trades',
        '_total_buy_volume',
        '_total_sell_volume',
