...
STATES={'NO_LOG':False, 'NO_COMMENT':False, 'NO_SAVE':False, 'NO_PLOT':False,'START':20130101,
        'db_host':'127.0.0.1','db_port':3306,'db_user':'root','db_password':'Kingwila98'}


class HomePlace(object):
    __slots__ = ['daily_data_file','factor_data_file','barra_data_file','update_data_file',
...
@cachier()
def read_daily(path=None,open=0,close=0,high=0,low=0,tr=0,sharenum=0,volume=0,unadjust=0,start=STATES['START']):
...
def read_industry_prices(day=None,monthly=True):
    ...
    if day is None:
        day=STATES['START']
    df=pd.read_feather(homeplace+'各行业行情数据.feather').set_index('date')
    if monthly:
        df=df.resample('M').last()
    return df
...
class pure_fall():
    ...
    def minute_to_daily(self, func,add_priclose=False,add_tr=False,start_date=10000000,end_date=30000000,update=0):
        ...
        self.daily_factors=self.daily_factors[self.daily_factors.index>=pd.Timestamp(str(STATES['START']))]
    ...
    def get_daily_factors(self, func, whole=False,add_priclose=False,add_tr=False,start_date=10000000,end_date=30000000):
        ...
        try:
            ...
            if self.daily_factors.index.max()<now_minute_data:
                ...
                end_date_update=int(datetime.datetime.strftime(now_minute_data,'%Y%m%d'))
                if whole:
    ...
    @kk.desktop_sender(title='嘿，分钟数据处理完啦～🎈')
    def get_daily_factors_alter(self,func,start_date=10000000,end_date=30000000)->pd.DataFrame:
...
class pure_fall_frequent(object):
    '''对单只股票单日进行操作'''
    def __init__(self,factor_file:str,startdate:int=None,enddate:int=None,kind:str='stock'):
        self.kind=kind
        #连接clickhouse
    ...
    @kk.desktop_sender(title='嘿，分钟数据处理完啦～🎈')
    def get_daily_factors(self,func,fields:str='*',chunksize:int=250,show_time=True,tqdm_inside=True):
        '''对单只股票单日进行操作'''
        the_func=partial(func)
...
class pure_coldwinter():
    ...
    def get_corr(self):
        ...
        self.corr_by_step = self.corr_pri.groupby(['date']).apply(lambda x: x.corr().head(1))
        self.__corr = self.corr_by_step.mean()
        self.__corr=self.__corr.rename(index={
            'fac':'因子自身','beta':'贝塔','booktoprice':'估值','leverage':'杠杆','earningsyield':'盈利','growth':'成长',
            'liquidity':'流动性','momentum':'动量','residualvolatility':'波动率','size':'市值','nonlinearsize':'非线性市值'
        })
        # self.__corr.index=['因子自身','贝塔','估值','杠杆',
        #                    '盈利','成长','流动性','反转','波动率',
        #                    '市值','非线性市值']
...
class sqlConfig(object):
    def __init__(self,db_name:str=None,db_user:str=STATES['db_user'],db_host:str=STATES['db_host'],db_port:int=STATES['db_port'],db_password:str=STATES['db_password']):
    ...
    def show_tables_old(self,db_name:str=None,full=True):
        ...
        mycursor=self.login()
        if full:
            return mycursor.execute(f"select * from information_schema.tables where TABLE_SCHEMA={f'{db_name}'}")
        else:
            return mycursor.execute(f"select TABLE_NAME from information_schema.tables where TABLE_SCHEMA={f'{db_name}'}")
    ...
    def show_tables(self,db_name:str=None,full:bool=True):
        ...
        if db_name is None:
            db_name=self.db_name
        if full:
            res=self.get_data_sql_order(f"select * from information_schema.tables where TABLE_SCHEMA='{db_name}'")
        else:
            res=self.get_data_sql_order(f"select TABLE_NAME from information_schema.tables where TABLE_SCHEMA='{db_name}'")
    ...
    def show_databases(self,user_only:bool=True,show_number:bool=True)->list:
        '''显示数据库信息'''
        mycursor=self.login()
        res=self.get_data_sql_order('select SCHEMA_NAME from information_schema.schemata')
        ...
        if show_number:
            for i in res:
                di[i]=mycursor.execute(f"select * from information_schema.tables where TABLE_SCHEMA='{i}'")
            return di
        else:
            return res
    ...
    def get_data_old(self,table_name:str,fields:str=None,startdate:int=None,enddate:int=None,show_time=False)->pd.DataFrame:
        '''
        从数据库中读取数据，
    ...
    def get_data(self,table_name:str,fields:str=None,startdate:int=None,enddate:int=None,show_time=False)->pd.DataFrame:
        '''
        从数据库中读取数据，
        table_name为表名，数字开头的加键盘左上角的`符号，形如`000001.SZ`或`20220717`
        fields形如'date,close,open.amount'，不指定则默认读入所有列
...
INSERT INTO minute_data.minute_data (date, code, open, high, low, close, amount, money, num) VALUES
                                         (0,0,0,0,0,0,0,0,0);
    '''
    def __init__(self,database_name:str,database_host:str='127.0.0.1',database_user:str='default',database_password=''):
    ...
    def show_all_codes(self,table_name:str)->list:
        '''返回分钟数据中所有股票的代码'''
        df=self.get_data(f'select distinct(code) from {self.database_name}.{table_name}').sort_values('code')
        return [i for i in list(df.code) if i!='0']

    def show_all_dates(self,table_name:str,mul_100=False)->list:
        '''返回分钟数据中所有日期'''
        df=self.get_data(f'select distinct(date) from {self.database_name}.{table_name}').sort_values('date')
...
def database_update_minute_files(startdate:str=None,enddate:str=None,to_mat=True,to_clickhouse=False):
    '''
    ...
    if to_clickhouse:
        ...
        minute_data_sql['amount']=(minute_data_sql['amount']*100).astype(int).ffill()
        ...
        minute_data_sql.to_sql('minute_data',chc.engine,if_exists='append',index=False)
...
def database_update_minute_data_to_clickhouse(kind:str)->pd.DataFrame:
    ...
    ts=rqdatac.get_price(
        codes,start_date=start_date,end_date=end_date,frequency='1m',
        fields=['volume','total_turnover','high','low','close','open'],adjust_type='none',
        skip_suspended=False,market='cn',expect_df=True,time_slice=None)
    #调整数据格式
    ts=ts.reset_index()
    ts=ts.rename(columns={'order_book_id':'code','datetime':'date','volume':'amount','total_turnover':'money'})
    ts=ts.sort_values(['code','date'])
    ts.date=ts.date.dt.strftime('%Y%m%d').astype(int)
    ts=ts.groupby(['code','date']).apply(lambda x:x.assign(num=list(range(1,x.shape[0]+1))))
    ts=(np.around(ts.set_index('code'),2)*100).ffill().dropna().astype(int).reset_index()
...
def database_update_minute_data_to_mysql(kind:str)->pd.DataFrame:
        ...
        for code in codes:
            ...
            try:
                dfi.drop(columns=['code']).to_sql(
                    name=code,con=sqls.engine,if_exists='append',index=False,
            ...
            try:
                ...
                try:
                    if sqlsa.get_data(date).shape[0]==0:
                        dfi.drop(columns=['date']).to_sql(
                            name=str(date),con=sqlsa.engine,if_exists='replace',index=False,
                            dtype={'code':VARCHAR(9),'open':INT,'high':INT,'low':INT,'close':INT,'amount':BIGINT,'money':BIGINT,'num':INT})
                except Exception:
                    fails.append(date)
    ...
    #指数
        ...
        for code in codes:
            ...
            try:
                dfi.drop(columns=['code']).to_sql(
                    name=code,con=sqli.engine,if_exists='append',index=False,
                    dtype={'date':INT,'open':INT,'high':INT,'low':INT,'close':INT,'amount':BIGINT,'money':BIGINT,'num':INT})
            except Exception:
                try:
            ...
            try:
                ...
                try:
                    if sqlia.get_data(date).shape[0]==0:
                        dfi.drop(columns=['date']).to_sql(
                            name=str(date),con=sqlia.engine,if_exists='replace',index=False,
                            dtype={'code':VARCHAR(9),'open':INT,'high':INT,'low':INT,'close':INT,'amount':BIGINT,'money':BIGINT,'num':INT})
                except Exception:
                    fails.append(date)
...
def download_single_day_style(day):
    '''更新单日的数据'''
    try:
        style=pro.RMExposureDayGet(trade_date=str(day),fields='tradeDate,ticker,BETA,MOMENTUM,SIZE,EARNYILD,RESVOL,GROWTH,BTOP,LEVERAGE,LIQUIDTY,SIZENL')
        time.sleep(1)
        return style
    except Exception:
        time.sleep(60)
...
def database_update_barra_files():
    ...
    tradedates=sorted(list(set(df0.trade_date)))
    style_names=['beta','momentum','size','residualvolatility','earningsyield','growth','booktoprice','leverage','liquidity','nonlinearsize']
    ...
    for t in tqdm.tqdm(tradedates):
        ...
        style=style.rename(columns={'earnyild':'earningsyield','tradedate':'date','ticker':'code','resvol':'residualvolatility','btop':'booktoprice','sizenl':'nonlinearsize','liquidty':'liquidity'})
...
def download_single_index(index_code:str):
    if index_code=='000300.SH':
        ...
        file=index_code
    try:
        df=pro.index_daily(ts_code=index_code).sort_values('trade_date').rename(columns={'trade_date':'date'})
        df=df[['date','close']]
        df.date=pd.to_datetime(df.date,format='%Y%m%d')
        df=df.set_index('date')
        df=df.resample('M').last()
...
def download_single_industry_price(ind):
    try:
        df=pro.swindex_daily(code=ind)[['trade_date','close']].set_index('trade_date')
        df.columns=[ind]
        time.sleep(1)
        return df
    except Exception:
...
def database_update_industry_member():
    ...
    dfs=dfs.fillna(0)
    dfs.reset_index(drop=True).to_feather(homeplace.daily_data_file+'申万行业2021版哑变量.feather')
...
def download_single_index_member(code):
    ...
    if start_date>=end_date:
        ...
        start_date,end_date=datetime.datetime.strftime(start_date,'%Y%m%d'),datetime.datetime.strftime(end_date,'%Y%m%d')
        logger.info(f'{INDEX_DICT[code]}月成分股上次更新到{start_date},本次将更新到{end_date}')
        try:
            a=pro.index_weight(index_code=code,start_date=start_date,end_date=end_date)
            if a.shape[0]==0:
                logger.info(f'{INDEX_DICT[code]}月成分股无需更新，上次已经更新到了{start_date}')
            else:
                time.sleep(1)
                ...
                a=a.groupby('con_code').resample('M').last().drop(columns=['con_code']).reset_index()
                a=a.assign(num=1)
                a=a[['trade_date','con_code','num']].rename(columns={'trade_date':'date','con_code':'code'}).pivot(columns='code',index='date',values='num')
